...
int countPrint = 0;
uint16_t elReadFromMatlab = 0;
uint16_t azReadFromMatlab = 0;
void loop() {
while(1)
{
  serialEvent();
  ...
  {
    elReadFromMatlab = (inputString[2] - 48) * 10000
                      +(inputString[3] - 48) * 1000
                      +(inputString[4] - 48) * 100
                      +(inputString[5] - 48) * 10
                      +(inputString[6] - 48);
azReadFromMatlab = (inputString[7] - 48) * 10000
                      +(inputString[8] - 48) * 1000
                      +(inputString[9] - 48) * 100
                      +(inputString[10] - 48) * 10
                      +(inputString[11] - 48);

    // here elReadFromMatlab = 9000 max, 0 min, azReadFromMatlab = 36000 max 0 min
    // code goes here

    switch (inputString[1] - 48)
    {
      case MODE_TEST_1: // 0
        {
        ...
        case MODE_HOMING_AZ_MANUALLY: // 5
          ...
          case MODE_PC_EL:  //  7  Mode verify angle by Smartphone via serial //
          {
            motorEl.homingDone = 1; // Comment this with physcial GS
            motorEl.isControlled = 1;

            if ((motorEl.homingDone == 1) && (motorEl.isControlled == 1))
            {
                motorEl.desiredPulse = (elReadFromMatlab*2222.22) / 100 ;

            }
            break;
          }

          case MODE_PC_AZ:  //  8  Mode verify angle by Smartphone via serial //
          {
            motorAz.homingDone = 1; // Comment this with physcial GS
            motorAz.isControlled = 1;
            if ((motorAz.homingDone == 1) && (motorAz.isControlled == 1))
            {
                motorAz.desiredPulse = (azReadFromMatlab * 444.44) /100;
    ...
    Serial.print("el,");
    Serial.print(motorEl.currentPulse/2222.22);
    Serial.print(",");
    Serial.print("az,");
    Serial.print(motorAz.currentPulse/444.44);
    Serial.print(",");
    Serial.print(EL_END);
    Serial.print(",");
    Serial.print(AZ_END);
    Serial.print(",");
    Serial.print(motorEl.homingDone);
    Serial.print(",");
    Serial.print(motorAz.homingDone);
    Serial.print(",");

    Serial.println(MODE);

     //Serial.println("adadsad");
